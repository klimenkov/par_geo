...
! используется столбцы ось х - правая ветка параболоида obr.xls
! ось у yrad.xls, отсортированный по возрастанию
!	open(333,file='en.xls')
!     rf0right - точки на круге
!     psf0right - свободная поверхность 
...
!      Y=0.
 ...
 !    YSILA=Y
	...
	Y=0.
      ...
      IPR=1
!      IPR=0
!     PSZ() - начальное отклонение по высоте     
      PSZ(1)=0.25
      PSZ(1)=0.25
      PSZ(2)=0.15
      PSZ(3)=0.15
...
!      PSZ(I)=0.15
   10 CONTINUE
      ...
      NF1=1
      NF2=4
!     Задаем  функции для окружного номера М=1 и 1-го номера по радиусу 
!     F1=R1*SINX ;F2=R1*COSX
!     для окружного номера М=1 и 4-го номера по радиусу:   F9=R4*SINX ;F10=R4*COSX
 	
	CALL VFORMG(H,W,M,NF1,NF2,IPR)
      M=0
      NF1=2
      NF2=6
!     для окружного номера М=0
!     для 2-го радиального номера F3=R2
!     для 6-го радиального номера F8=R6
      CALL VFORMG(H,W,M,NF1,NF2,IPR)
      M=2
      NF1=3
      NF2=0
!     для окружного номера М=2 и 3-го номера по радиусу 
!     F4=R3*SIN2X ;F5=R3*COS2X  
      CALL VFORMG(H,W,M,NF1,NF2,IPR)
      M=3
      NF1=5
!     для окружного номера М=3 и 5-го номера по радиусу 
!     F6=R5*SIN3X ;F7=R5*COS3X  
      CALL VFORMG(H,W,M,NF1,NF2,IPR)
!     NN(10)- номер функции R по радиусу
!     MM(10)- номер тригонометрической части 
!     MN(NF1) - окружные номера,соответствующие номерам по радиусу NF1 и NF2
!     F(I)=R(NN(I))*TPS(MM(I))
!     TPS() - определяет тригонометрическую часть функции F
      NN(1)=1
      MM(1)=1
!     Соответствует M=1;NF1=1;F1=R1*SINX
      NN(2)=1
      MM(2)=2
!     Соответствует M=1;NF1=1;F2=R1*COSX
      NN(3)=2
      MM(3)=3
!     Соответствует M=0;NF1=2;F3=R2
      NN(4)=3
      MM(4)=4
!     Соответствует M=2;NF1=3;F4=R3*SIN2X
      NN(5)=3
      MM(5)=5
!     Соответствует M=2;NF1=3;F5=R3*COS2X
      NN(6)=6
      MM(6)=3
!     Соответствует M=0;NF2=6;F6=R6
      NN(7)=5
      MM(7)=6
!     Соответствует M=3;NF2=5;F7=R5*SIN3X
      NN(8)=5
      MM(8)=7
!     Соответствует M=3;NF2=5;F8=R5*COS3X
      NN(9)=4
	MM(9)=1
!     Соответствует M=1;NF2=4;F9=R4*SINX
      NN(10)=4
      MM(10)=2
!     Соответствует M=1;NF2=4;F9=R4*COSX
      MN(1)=1
!     Соответствует M=1;NF1=1
      MN(2)=0
!     Соответствует M=0;NF1=2
      MN(3)=2
!     Соответствует M=2;NF1=3
      MN(4)=1
!     Соответствует M=1;NF2=4
      MN(5)=3
!     Соответствует M=3;NF2=5
      MN(6)=0
!     Соответствует M=0;NF2=6
      RETURN
      END
...
!   15 FORMAT(3X,F10.4,'=T')
      ...
      !     Задаем уравнение образующей
	DOUBLE PRECISION FUNCTION FG (X)
      DOUBLE PRECISION X,H,R,D,D1,F0,D2,Z1,Hreal,R0,ag,bg
      COMMON/GEO/ H,R,D,D1,D2,Z1,Hreal,R0
	common/GIPER/ag,bg
!     Y=x**2      
	 D2=DSQRT(X+H)
...
!     Первая производная
      ...
      DOUBLE PRECISION X,H,R,D,D1,F0P,D2,Z1,Hreal,R0,ag,bg
      COMMON/GEO/ H,R,D,D1,D2,Z1,Hreal,R0
	common/GIPER/ag,bg
!     Y=x**2 
!       F0P=0.5/DSQRT(X+H)    
...
C     SOME DIFFEFENCIALS FROM GARMONIC POLINOMS FOR GEO-BLOCK
      ...
      COMMON/FORMA/ PSF(25,6),PS0(25),PSZ(6),AF0,AF1
      COMMON/MEN/ MM(10),NN(10),MN(6)
!     FS(I,J,AL,BE,K,NF) - I - номер производной по Z;J - номер производной по R;
!     k - номер по радиусу; NF - окружной номер
      Z=BE*H
      R=AL*FG(Z)
      E=Z-PSZ(K)
!     PSZ(K) - смещение от искусственно завышенной высоты
!     E - высота на срезе
!     NF - кол-во функций в разложении ряда(определяет размер суммирования)
...
!     М - окружной номер
      L=IJ+M-1
!     Пересчет индексов для гармонических полиномов W
      IF(M.EQ.0) L=L+1
!     При нулевом номере повышаем индекс
...
C      IF(I.EQ.3.AND.J.EQ.1) W=WR3Z (M,L,R,E)
    1 S=S+W*PSF(IJ,K)
!     PSF(IJ,K) - массив, который определяется из решения задачи на с.значения и собств. вектора,
!     вычисляется в VFORMG() и определяет саму ф(при i=j=0) и все производные
      FS=S
      RETURN
      END
...
!  655 format('Эпюры h(96)= y(77)= ', 2F12.8)
!      write (889,302), ALAM
